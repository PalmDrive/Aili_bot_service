  ...
  onRecommend(data, user) {
    ...
    return this._getRecommendedMediaId(user.get('open_id'))
      .then(mediaIds => {
        ...
        let cql = `select * from Media where objectId not in ${mediaIdsStr} and isActive = true order by createdAt ASC limit 0, 1000`;

        if (this.client.get('name') === 'Ailingual' /* 'Flipboard' */) {
          cql = `select * from Media where objectId not in ${mediaIdsStr} and isActive = true and source = 'Flipboard' order by createdAt DESC limit 0, 1000`;
        }
