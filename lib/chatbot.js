...
const msgTypes = ['article', 'video', 'image', 'voice', 'music'];
...
class Chatbot {
  ...
  init() {
    return new Promise((resolve, reject) => {
      if (this.type === 'WECHAT') {
        ...
        return wechat.getAccessToken(clientId)
          .then(token => {
  ...
  sendWechatMessageWithMedia(medium, user) {
    const accessToken = this.accessToken,
    ...
    if (medium) {
      ...
      const responseData = this._getResDataFromMedia(medium, touser),
            msgs = medium.map(m => {
  ...
  onRecommend(data, user) {
    ...
    return this._getRecommendedMediaId(user.get('open_id'))
      .then(mediaIds => {
        ...
        let cql = `select * from Media where objectId not in ${mediaIdsStr} and isActive = true order by createdAt ASC limit 0, ${itemsCount}`;

        if (this.client.get('name') === 'Ailingual' /* 'Flipboard' */) {
          cql = `select * from Media where objectId not in ${mediaIdsStr} and isActive = true and source = 'Flipboard' and type = 'article' order by createdAt DESC limit 0, ${itemsCount}`;
        }

        return leanCloud.AV.Query.doCloudQuery(cql);
      })
      .then(data => {
        const groupedMediumDict = _.groupBy(data.results, media => media.get('type')),
              groupedMedium = _.reduce(groupedMediumDict, (memo, arr) => {
                memo = memo.concat(arr);
                return memo;
              }, []);
        return Promise.all(groupedMedium.map(medium => {
          return this.sendWechatMessageWithMedia(medium, user);
        }));
      });
      // .then(media => {
